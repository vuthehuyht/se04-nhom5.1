 ...
 *
 */
class CENOTEFramework_Customize extends CENOTEFramework_Abstract {
	...
	public function __construct($options) {
		...
		$this->options = apply_filters('cenote_customize_options', $options);

		do_action('cenote_customize_options_config', $this->options);
	...
	public static function instance($options = array()) {
		if (is_null(self::$instance) && CENOTE_ACTIVE_CUSTOMIZE) {
	...
	public function customize_register($wp_customize) {
		...
		cenote_locate_template('functions/customize.php');
		...
		do_action('cenote_customize_register', $wp_customize);
	...
	public function add_section($wp_customize, $value, $panel = false) {
		...
		foreach ($sections as $section) {
			...
			foreach ($section['settings'] as $setting) {
				...
				$setting_name = CENOTE_CUSTOMIZE . '[' . $setting['name'] . ']';
				...
				$wp_customize->add_setting($setting_name,
					wp_parse_args($setting, array(
							...
							'sanitize_callback' => 'cenote_sanitize_clean',
				...
				$control_args = wp_parse_args($setting['control'], array(
					'unique' => CENOTE_CUSTOMIZE,
					'section' => $section['name'],
					'settings' => $setting_name,
					'priority' => $setting_priority,
				));

				if ($control_args['type'] == 'cenote_field') {
