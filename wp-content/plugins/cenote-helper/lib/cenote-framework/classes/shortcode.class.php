 ...
 *
 */
class CENOTEFramework_Shortcode_Manager extends CENOTEFramework_Abstract {
	...
	public function __construct($options) {
		...
		$this->options = apply_filters('cenote_shortcode_options', $options);
		$this->exclude_post_types = apply_filters('cenote_shortcode_exclude', $this->exclude_post_types);
		...
		if (!empty($this->options)) {
			...
			$this->addAction('wp_ajax_cenote-get-shortcode', 'shortcode_generator', 99);
	...
	public static function instance($options = array()) {
		if (is_null(self::$instance) && CENOTE_ACTIVE_SHORTCODE) {
	...
	public function media_shortcode_button($editor_id) {
		...
		if (!in_array($post_type, $this->exclude_post_types)) {
			echo '<a href="#" class="button button-primary cenote-shortcode" data-editor-id="' . $editor_id . '">' . __('Add Shortcode', 'cenote-helper') . '</a>';
		}
	...
	public function shortcode_dialog() {
		?>
		<div id="cenote-shortcode-dialog" class="cenote-dialog" title="<?php _e('Add Shortcode', 'cenote-helper'); ?>">
			<div class="cenote-dialog-header">
				<select class="<?php echo (is_rtl()) ? 'chosen-rtl ' : ''; ?>cenote-dialog-select"
						data-placeholder="<?php _e('Select a shortcode', 'cenote-helper'); ?>">
					<option value=""></option>
					<?php
			...
			<div class="cenote-dialog-load"></div>
			<div class="cenote-insert-button hidden">
				<a href="#"
				   class="button button-primary cenote-dialog-insert"><?php _e('Insert Shortcode', 'cenote-helper'); ?></a>
			</div>
		</div>
		<?php
	}
	...
	public function shortcode_generator() {
		...
		$request = cenote_get_var('shortcode');
		...
		if (isset($shortcode['fields'])) {
			...
			foreach ($shortcode['fields'] as $key => $field) {
				...
				echo cenote_add_element($field, $field_default, 'shortcode');
		...
		if (isset($shortcode['clone_fields'])) {
			...
			echo '<div class="cenote-shortcode-clone" data-clone-id="' . $clone_id . '">';
			echo '<a href="#" class="cenote-remove-clone"><i class="fa fa-trash"></i></a>';
			...
			echo '<div class="cenote-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> ' . $shortcode['clone_title'] . '</a></div>';
