 ...
 *
 */
class CENOTEFramework_Taxonomy extends CENOTEFramework_Abstract {
	...
	public function __construct($options) {
		...
		$this->options = apply_filters('cenote_taxonomy_options', $options);
	...
	public static function instance($options = array()) {
		if (is_null(self::$instance) && CENOTE_ACTIVE_TAXONOMY) {
	...
	public function add_taxonomy_fields() {
		...
		foreach ($this->options as $option) {
			...
			$get_taxonomy = cenote_get_var('taxonomy');
	...
	public function render_taxonomy_form_fields($term) {
		...
		$cenote_errors = get_transient('cenote-taxonomy-transient');

		wp_nonce_field('cenote-taxonomy', 'cenote-taxonomy-nonce');

		echo '<div class="cenote-framework cenote-taxonomy cenote-taxonomy-' . $classname . '-fields">';
		...
		foreach ($this->options as $option) {
			...
			if ($taxonomy == $option['taxonomy']) {
				...
				foreach ($option['fields'] as $field) {
					...
					echo cenote_add_element($field, $elem_value, $option['id']);
	...
	public function save_taxonomy($term_id) {
		...
		if (wp_verify_nonce(cenote_get_var('cenote-taxonomy-nonce'), 'cenote-taxonomy')) {

			$errors = array();
			$taxonomy = cenote_get_var('taxonomy');
			...
			foreach ($this->options as $request_value) {
				...
				if ($taxonomy == $request_value['taxonomy']) {
						...
						foreach ($request_value['fields'] as $field) {
							...
							if (isset($field['type']) && isset($field['id'])) {
								...
								if (has_filter('cenote_sanitize_' . $sanitize_type)) {
									$request[$field['id']] = apply_filters('cenote_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields']);
								}

								// validate options
								if (isset($field['validate']) && has_filter('cenote_validate_' . $field['validate'])) {

									$validate = apply_filters('cenote_validate_' . $field['validate'], $field_value, $field, $request_value['fields']);
					...
					$request = apply_filters('cenote_save_taxonomy', $request, $request_key, $term_id);
			...
			set_transient('cenote-taxonomy-transient', $errors, 10);
