 ...
 *
 */
class CENOTEFramework_Option_typography extends CENOTEFramework_Options {
  ...
  public function output() {
    ...
    $websafe_fonts = apply_filters('cenote_websafe_fonts', array(
      'Arial',
      'Arial Black',
      'Comic Sans MS',
      'Impact',
    ...
    if(is_object($google_json)) {
      ...
      echo '<select name="'. $this->element_name('[family]') .'" class="'. $is_chosen . $chosen_rtl .'cenote-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action('cenote_typography_family', $family_value, $this);

      echo '<optgroup label="'. __('Web Safe Fonts', 'cenote-helper') .'">';
      ...
      echo '<optgroup label="'. __('Google Fonts', 'cenote-helper') .'">';
      ...
      echo '<input type="text" name="'. $this->element_name('[font]') .'" class="cenote-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';
